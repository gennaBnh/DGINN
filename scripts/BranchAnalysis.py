...
def bppBranch(OPBFile, outDir, baseName, alnFile, alnFormat, treeFile):
	...
	print("One Per Branch (BIO++)")
	print("OPB parameter file: {:s}".format(OPBFile))
	...
	# running bppml
	print("Running Branch optimization")
...
#	  logger.debug(argsOPB)
	  ...
	  if p<0.05:
	    print("Node {:d} is interesting (w = {:f})".format(idi, dparam["YNGP_M2.omega2_%d"%mod]))
	fresbranch.close()
	return(outParams)
	...
	with open(outParams, "r") as op:
		for line in op.readlines():
			if line.startswith("model") and line.endswith(")\n"):
				...
				if w > 1:
					print("Node {:d} is interesting (w = {:f})".format(node, w))
					lPSNodes.append(node)
				else:
					lNSNodes.append(node)
		op.close()
	print("Nodes under positive selection {}".format(lPSNodes))
	print("Nodes under neutral or negative selection {}".format(lNSNodes))
...
def bppBranchSite(GNHFile, lPSNodes, outDir, baseName, alnFile, alnFormat, treeFile):
	...
	print("General Non Homogenous on branches with w > 1 (BIO++)")
	print("GNH parameter file: {:s}".format(GNHFile))
	...
	for node in lPSNodes:
		...
		# running bppml
		print("Running Pseudo Branch-Site optimization")
...
def memeBranchSite(aln, cladoFile, outDir, baseName):
	...
	print("Episodic selection (MEME, HYPHY)")
