...
def cutLongBranches(aln, dAlnTree, nbSp, LBOpt, logger):
	...
	print("Long branches will be evaluated through the {} method (factor {})".format(LBOpt, factor))
	nbSp = int(nbSp)	
	...
	if len(matches) > 0:
		...
		for node in matches:
			...
			if len(dNewAln) > nbSp - 1:
				...
				pass
				print("Sequences {} will not be considered for downstream analyses as they do not compose a large enough group.".format(dNewAln.keys()))
		...
		if len(dID2Seq) > nbSp - 1:
			with open(alnLeft, "w") as fasta:
				...
				print("\tNew alignment:%s"%{alnLeft})
				fasta.close()	  
			dAlnTree[alnLeft] = ""
		else:
			pass
			print("Sequences in {} will not be considered for downstream analyses as they do not compose a large enough group.".format(dID2Seq.keys()))
...
if __name__ == "__main__" :	
	...
	if sys.argv[2] == "phyMLTree":
		...
		if parameters["duplication"]:
			...
			dAlTree = tree.treeTreatment(data, data["dAlTree"], parameters["nbspecies"], parameters["phymlOpt"], sys.argv[3])
			data["dAlTree"] = dAlTree
		...
		if parameters["recombination"] :
			...
			data["dAlTree"] = gardRecomb(data, data["dAlTree"], parameters["hostfile"], sys.argv[3])
